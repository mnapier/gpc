...
GPC's RTDN decoder covers all documented event types: 18 subscription (1-13, 17-20, 22), 2 OTP, plus voided-purchase and test-notification handlers.
...
::: info Fixed
`gpc vitals errors reports` now calls the correct `GET /apps/{pkg}/errorReports:search` endpoint with an `errorIssueId` filter.
:::
...
#### Formal deprecation wave (May 21, 2025)
...
- **Soft-deprecated (no formal deadline yet)**: Google's discovery doesn't flag these but their descriptions point callers at v2 equivalents:
...
- **`inappproducts` for one-time products**: Google deprecated this endpoint for OTP management (shutdown August 2027). GPC routes through `monetization.oneTimeProducts` internally and emits a warning on legacy IAP commands.

GPC currently emits deprecation warnings on `purchases.subscriptions.get` and legacy `inappproducts` IAP operations. Expanding warning coverage to the remaining soft-deprecated surfaces is a candidate for a future release.
